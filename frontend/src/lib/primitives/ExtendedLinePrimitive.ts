...
import type { ExtendedLineDrawing } from "../drawings";
...
class ExtendedLineRenderer implements IPrimitivePaneRenderer {
    ...
    constructor(drawing: ExtendedLineDrawing, seriesRef: React.RefObject<any>, chartRef: React.RefObject<any>, isSelected: boolean) {
    ...
    draw(target: any): void {
        target.useBitmapCoordinateSpace(({ context, bitmapSize, verticalPixelRatio, horizontalPixelRatio }: {
            ...
            const yLEdge = y1Px - slope * x1Px;
            const yRedge = y1Px + slope * (bitmapSize.width - x1Px);
            ...
            context.moveTo(0, yLEdge);
            context.lineTo(bitmapSize.width, yRedge);
            context.stroke();
...
class ExtendedLinePaneView implements IPanePrimitivePaneView {
    private _drawing: ExtendedLineDrawing;
    private _seriesRef: React.RefObject<any>;
    private _chartRef: React.RefObject<any>;
    private _isSelected: boolean;
    ...
    renderer(): IPrimitivePaneRenderer {
        return new ExtendedLineRenderer(this._drawing, this._seriesRef, this._chartRef, this._isSelected);
    }
}

export class ExtendedLinePrimitive implements IPanePrimitive {
    private _drawing: ExtendedLineDrawing;
    private _seriesRef: React.RefObject<any>;
    private _chartRef: React.RefObject<any>;
    private _isSelected: boolean;
    ...
    paneViews() {
        return [new ExtendedLinePaneView(this._drawing, this._seriesRef, this._chartRef, this._isSelected)]
    }

    update(drawing: ExtendedLineDrawing, isSelected: boolean) {
        this._drawing = drawing;
        this._isSelected = isSelected;
        this._requestUpdate?.();
    }
