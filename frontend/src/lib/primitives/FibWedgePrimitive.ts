...
import type { FibWedgeDrawing } from "../drawings";
import type React from "react";
import { lineDashForStyle } from "../drawingUtils";

class FibWedgeRenderer implements IPrimitivePaneRenderer {
    private _drawing: FibWedgeDrawing;
    private _seriesRef: React.RefObject<any>;
    private _chartRef: React.RefObject<any>;
    private _isSelected: boolean;

    constructor(drawing: FibWedgeDrawing, seriesRef: React.RefObject<any>, chartRef: React.RefObject<any>, isSelected: boolean) {
    ...
    draw(target: any): void {
        target.useBitmapCoordinateSpace(({ context, verticalPixelRatio, horizontalPixelRatio }: {
            ...
            const fanLevels = [0, ...this._drawing.levels.filter(l => l > 0 && l <= 1), 1];
            const outerRadius = this._drawing.levels[this._drawing.levels.length - 1] * baseRadius;
            for (const l of fanLevels) {
                const angle = startAngle + l * (endAngle - startAngle);
                const lineEndX = x1Px + outerRadius * Math.cos(angle);
                const lineEndY = y1Px + outerRadius * Math.sin(angle);
                context.beginPath();
...
class FibWedgePaneView implements IPanePrimitivePaneView {
    private _drawing: FibWedgeDrawing;
    private _seriesRef: React.RefObject<any>;
    private _chartRef: React.RefObject<any>;
    private _isSelected: boolean;
    ...
    renderer(): IPrimitivePaneRenderer {
        return new FibWedgeRenderer(this._drawing, this._seriesRef, this._chartRef, this._isSelected);
    }
}

export class FibWedgePrimitive implements IPanePrimitive {
    private _drawing: FibWedgeDrawing;
    private _seriesRef: React.RefObject<any>;
    private _chartRef: React.RefObject<any>;
    private _isSelected: boolean;
    ...
    paneViews() {
        return [new FibWedgePaneView(this._drawing, this._seriesRef, this._chartRef, this._isSelected)]
    }

    update(drawing: FibWedgeDrawing, isSelected: boolean) {
        this._drawing = drawing;
        this._isSelected = isSelected;
        this._requestUpdate?.();
    }
