...
import type { TrendAngleDrawing } from "../drawings";
import type React from "react";
import { lineDashForStyle } from "../drawingUtils";

class TrendAngleRenderer implements IPrimitivePaneRenderer {
    private _drawing: TrendAngleDrawing;
    private _seriesRef: React.RefObject<any>;
    private _chartRef: React.RefObject<any>;
    private _isSelected: boolean;

    constructor(drawing: TrendAngleDrawing, seriesRef: React.RefObject<any>, chartRef: React.RefObject<any>, isSelected: boolean) {
    ...
    draw(target: any): void {
        target.useBitmapCoordinateSpace(({ context, verticalPixelRatio, horizontalPixelRatio }: {
            ...
            const radius = 30 * horizontalPixelRatio;
            ...
            context.beginPath();
            context.setLineDash([]);
            context.arc(x1Px, y1Px, radius, 0, angle);
            context.stroke();
            context.fillText(`${angleDeg}°`, x1Px + (radius + 4 * horizontalPixelRatio), y1Px - 4 * verticalPixelRatio);
...
class TrendAnglePaneView implements IPanePrimitivePaneView {
    private _drawing: TrendAngleDrawing;
    private _seriesRef: React.RefObject<any>;
    private _chartRef: React.RefObject<any>;
    private _isSelected: boolean;
    ...
    renderer(): IPrimitivePaneRenderer {
        return new TrendAngleRenderer(this._drawing, this._seriesRef, this._chartRef, this._isSelected);
    }
}

export class TrendAnglePrimitive implements IPanePrimitive {
    private _drawing: TrendAngleDrawing;
    private _seriesRef: React.RefObject<any>;
    private _chartRef: React.RefObject<any>;
    private _isSelected: boolean;
    ...
    paneViews() {
        return [new TrendAnglePaneView(this._drawing, this._seriesRef, this._chartRef, this._isSelected)]
    }

    update(drawing: TrendAngleDrawing, isSelected: boolean) {
        this._drawing = drawing;
        this._isSelected = isSelected;
        this._requestUpdate?.();
    }
