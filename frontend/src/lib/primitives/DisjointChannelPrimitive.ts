...
import type { DisjointChannelDrawing } from "../drawings";
import type React from "react";
import { lineDashForStyle } from "../drawingUtils";

class DisjointChannelRenderer implements IPrimitivePaneRenderer {
    private _drawing: DisjointChannelDrawing;
    private _seriesRef: React.RefObject<any>;
    private _chartRef: React.RefObject<any>;
    private _isSelected: boolean;

    constructor(drawing: DisjointChannelDrawing, seriesRef: React.RefObject<any>, chartRef: React.RefObject<any>, isSelected: boolean) {
    ...
    draw(target: any): void {
        target.useBitmapCoordinateSpace(({ context, verticalPixelRatio, horizontalPixelRatio }: {
            ...
            context.lineTo(x4Px, y4Px);
            context.lineTo(x1Px, y3Px);
            context.closePath();
...
class DisjointChannelPaneView implements IPanePrimitivePaneView {
    private _drawing: DisjointChannelDrawing;
    private _seriesRef: React.RefObject<any>;
    private _chartRef: React.RefObject<any>;
    private _isSelected: boolean;
    ...
    renderer(): IPrimitivePaneRenderer {
        return new DisjointChannelRenderer(this._drawing, this._seriesRef, this._chartRef, this._isSelected);
    }
}

export class DisjointChannelPrimitive implements IPanePrimitive {
    private _drawing: DisjointChannelDrawing;
    private _seriesRef: React.RefObject<any>;
    private _chartRef: React.RefObject<any>;
    private _isSelected: boolean;
    ...
    paneViews() {
        return [new DisjointChannelPaneView(this._drawing, this._seriesRef, this._chartRef, this._isSelected)]
    }

    update(drawing: DisjointChannelDrawing, isSelected: boolean) {
        this._drawing = drawing;
        this._isSelected = isSelected;
        this._requestUpdate?.();
    }
